...
- S04 공식 2026-06 ZIP과 대구 CSV 해시를 고정하고 118,357행에서 세 점포를 상호/지점명+도로명주소로 정확 매칭했다. 세 곳 모두 공식 중분류 `I201 한식`이다.
...
- 백엔드 33개 테스트와 프런트 production build를 통과했다.

## 미완료·미검증

- 필수: 최신 메뉴·주문조건 실증빙과 신한카드 실파일·사전·사용권이 없어 실제 데이터 API는 아직 통과하지 않았다.
- 공간 결합: 세 점포가 성내2동과 삼덕동에 걸쳐 있어 `동성로·중앙로역 일대`의 공식 경계·대표 행정동·반경은 선언하지 않았다. 카드 자료의 실제 공간 단위에 맞춘 결합 근거가 필요하다.
...
- 승인된 신한카드 집계 파일, 데이터 사전/코드표, 세 점포를 설명할 수 있는 공간 단위와 결합 근거, 실제 범위·기간·단위·측정 방식, 분석/반출/ZIP/화면·영상/공개 웹 권한.
- 위 자료로 데이터 API 통과 후 Gemini 승인자·시각·모델·전송 범위·총/일 한도. 키 값은 요청하지 않는다.

## 재개 위치

1. [S04_MATCH_REPORT.md](S04_MATCH_REPORT.md)의 완료된 점포 매칭은 유지하고 [data_candidates.json](../data/real/data_candidates.json)의 메뉴·카드 null 필드를 실제 근거로 채운다.
2. `.venv\Scripts\python.exe scripts\verify_s04_matches.py`와 `scripts\check_v32_readiness.py`로 원본 매칭·후보·권한·승인 상태를 확인한다.
